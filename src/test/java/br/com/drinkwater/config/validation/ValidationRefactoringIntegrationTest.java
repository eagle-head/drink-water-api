...
/**
 * Integration test demonstrating that the validation refactoring works correctly. This test
 * verifies that the shared ValidationErrorFormatter produces the same results as the original
 * duplicated code would have.
 */
@SpringJUnitConfig
class ValidationRefactoringIntegrationTest {
    ...
    void verifySharedValidationFormatterWorksForApplicationProperties() {
        ...
        ApplicationProperties validProps =
                new ApplicationProperties("test-app", "1.0.0", "development");

        // When: Validating using the shared formatter
        List<String> errors =
                validationErrorFormatter.formatValidationErrors(
                        validProps, "Application", validator);
    ...
    void verifySharedValidationFormatterWorksForInvalidProperties() {
        ...
        ApplicationProperties invalidProps =
                new ApplicationProperties(null, "1.0.0", "development");

        // When: Validating using the shared formatter
        List<String> errors =
                validationErrorFormatter.formatValidationErrors(
                        invalidProps, "Application", validator);
    ...
    void verifySharedValidationFormatterWorksForRuntimeConfiguration() {
        ...
        List<String> errors =
                validationErrorFormatter.formatValidationErrors(
                        validConfig, "Runtime Logging", validator);
    ...
    void verifyErrorFormattingConsistency() {
        ...
        Set<ConstraintViolation<ApplicationProperties>> directViolations =
                validator.validate(invalidProps);
    ...
    void demonstrateRefactoringEliminatesDuplication() {
        // This test demonstrates that both RuntimeConfigurationValidator and
        ...
        List<String> errors =
                validationErrorFormatter.formatValidationErrors(
                        config, "Test Component", validator);
