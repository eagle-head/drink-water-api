...
/**
 * Integration tests for User API without Docker containers.
 * Uses H2 in-memory database and mocked JWT decoder for environments where Docker is not available.
 */
@SpringBootTest(webEnvironment = RANDOM_PORT)
@Import(MockContainersConfig.class)
@ActiveProfiles("it-no-containers")
@SqlGroup({
        @Sql(scripts = {"/data-h2.sql"}, executionPhase = BEFORE_TEST_METHOD),
        @Sql(scripts = {"/reset-h2.sql"}, executionPhase = AFTER_TEST_METHOD)
})
public final class UserApiNoContainersIT {

    @LocalServerPort
    private int port;

    @Autowired
    private MockTestAuthProvider authProvider;
