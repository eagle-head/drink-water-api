...
import br.com.drinkwater.config.properties.ContainerProperties;
import br.com.drinkwater.config.properties.CorsProperties;
import br.com.drinkwater.config.properties.DatabaseProperties;
import br.com.drinkwater.config.properties.JacksonProperties;
 ...
 *
 * <p>This configuration ensures that all properties are loaded and validated at application startup
 * time. The properties are immutable after bootstrap and cannot be changed at runtime.
 *
 * <p>All property classes use record types to ensure immutability and include comprehensive
 * validation annotations to enforce fail-fast behavior.
 *
 * <p>The actual validation and fail-fast behavior is handled by:
 *
 * <ul>
 *   <li>Individual property classes with validation annotations
 *   <li>EnvironmentVariableValidator for comprehensive startup validation
 *   <li>Spring Boot's ConfigurationProperties validation
 * </ul>
 *
 * <p>The properties are automatically registered and injected by Spring Boot. This configuration
 * also provides the LoggingSystem bean for runtime configuration management.
 */
@Configuration
...
public class EnvironmentVariableConfiguration {
     ...
     *
     * <p>The LoggingSystem is required for runtime logging level changes and is not automatically
     * available as a bean in Spring Boot 3.x applications. This method retrieves the current
     * LoggingSystem instance and makes it available for dependency injection.
     *
     * <p>This bean is essential for runtime configuration management, allowing services to
    ...
    LoggingSystem loggingSystem() {
        ...
        if (loggingSystem == null) {
            throw new IllegalStateException(
                    "No LoggingSystem available. This is required for runtime logging configuration.");
        }
