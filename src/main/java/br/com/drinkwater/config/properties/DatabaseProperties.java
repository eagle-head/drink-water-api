...
/**
 * Immutable configuration properties for database connectivity and JPA settings. All properties are
 * loaded at bootstrap time and cannot be modified at runtime.
 */
...
public record DatabaseProperties(
        @NotBlank(message = "DATABASE_URL environment variable is required")
                @Pattern(
                        regexp = "^jdbc:.*",
                        message = "DATABASE_URL must be a valid JDBC URL starting with 'jdbc:'")
                String url,
        @NotBlank(message = "DATABASE_USERNAME environment variable is required")
                @Size(
                        min = 1,
                        max = 63,
                        message = "DATABASE_USERNAME must be between 1 and 63 characters")
                String username,
        @NotBlank(message = "DATABASE_PASSWORD environment variable is required")
                @Size(
                        min = 8,
                        message = "DATABASE_PASSWORD must be at least 8 characters for security")
                String password,
        ...
        @NotNull(message = "DATABASE_POOL_SIZE environment variable is required")
                ...
                @Max(
                        value = 100,
                        message = "DATABASE_POOL_SIZE cannot exceed 100 for resource management")
                Integer poolSize,
        ...
        @NotNull(message = "DATABASE_CONNECTION_TIMEOUT environment variable is required")
                ...
                @Max(
                        value = 300000,
                        message = "DATABASE_CONNECTION_TIMEOUT cannot exceed 300000ms (5 minutes)")
