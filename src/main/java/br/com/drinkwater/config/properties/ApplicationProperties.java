...
/**
 * Immutable configuration properties for core application settings. All properties are loaded at
 * bootstrap time and cannot be modified at runtime.
 */
@ConfigurationProperties(prefix = "app")
@Validated
public record ApplicationProperties(
        @NotBlank(message = "APP_NAME environment variable is required") String name,
        @NotBlank(message = "APP_VERSION environment variable is required") String version,
        @NotBlank(message = "APP_ENVIRONMENT environment variable is required")
                @Pattern(
                        regexp = "^(development|dev|staging|stage|production|prod)$",
                        message =
                                "APP_ENVIRONMENT must be one of: development, dev, staging, stage, production, prod")
                String environment) {
