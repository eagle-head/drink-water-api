...
/**
 * Immutable configuration properties for security settings. All properties are loaded at bootstrap
 * time and cannot be modified at runtime.
 */
@ConfigurationProperties(prefix = "security")
@Validated
public record SecurityProperties(String jwtSigningKey, String encryptionKey, String apiSecretKey) {
    ...
    public boolean areKeysValidForEnvironment(boolean isProduction) {
        ...
        return jwtSigningKey != null
                && !jwtSigningKey.trim().isEmpty()
                && encryptionKey != null
                && !encryptionKey.trim().isEmpty()
                && apiSecretKey != null
                && !apiSecretKey.trim().isEmpty();
    }
}
