...
/**
 * Immutable configuration properties for Spring Boot Actuator endpoints. All properties are loaded
 * at bootstrap time and cannot be modified at runtime.
 */
...
public record ActuatorProperties(
        ...
        @NotBlank(message = "ACTUATOR_BASE_PATH environment variable is required")
                @Pattern(
                        regexp = "^/[a-zA-Z0-9/_-]*$",
                        message =
                                "ACTUATOR_BASE_PATH must start with '/'"
        ...
        @NotBlank(message = "ACTUATOR_HEALTH_SHOW_DETAILS environment variable is required")
                @Pattern(
                        ...
                        message =
                                "ACTUATOR_HEALTH_SHOW_DETAILS must be one of: never, when-authorized, always")
                String healthShowDetails,
        @NotBlank(message = "ACTUATOR_HEALTH_SHOW_COMPONENTS environment variable is required")
                @Pattern(
                        regexp = "^(never|when-authorized|always)$",
                        message =
                                "ACTUATOR_HEALTH_SHOW_COMPONENTS must be one of: never, when-authorized, always")
                String healthShowComponents) {

    // Valid actuator endpoints
    private static final Set<String> VALID_ENDPOINTS =
            Set.of(
                    "health",
                    "info",
                    "metrics",
    ...
    private static final Set<String> PRODUCTION_SAFE_ENDPOINTS =
    ...
    public boolean isProductionSafe() {
        return PRODUCTION_SAFE_ENDPOINTS.containsAll(endpoints)
                && "never".equals(healthShowDetails);
    }
    ...
    public boolean hasDevelopmentEndpoints() {
        return endpoints.stream()
                .anyMatch(endpoint -> !PRODUCTION_SAFE_ENDPOINTS.contains(endpoint));
    }
