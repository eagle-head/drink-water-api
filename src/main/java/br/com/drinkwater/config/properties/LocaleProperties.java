...
/**
 * Immutable configuration properties for locale and internationalization settings. All properties
 * are loaded at bootstrap time and cannot be modified at runtime.
 */
@ConfigurationProperties(prefix = "locale")
@Validated
public record LocaleProperties(
        @NotBlank(message = "DEFAULT_LOCALE environment variable is required") String defaultLocale,
    ...
    public List<Locale> getSupportedLocales() {
        return Objects.requireNonNull(supported.stream().map(this::parseLocale).toList());
    }
