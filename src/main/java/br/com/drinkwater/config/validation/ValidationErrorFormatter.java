 ...
 *
 * <p>This component provides centralized validation error formatting to eliminate code duplication
 ...
 *
 * <p>The formatted error messages follow the pattern: {@code [ComponentName] propertyPath:
 * validationMessage}
 *
 * <p>Example usage:
 *
...
public class ValidationErrorFormatter {
    ...
    /**
     * Validates a configuration object and formats any validation errors into standardized error
     * messages.
     *
     * <p>This method uses Jakarta Bean Validation to validate the provided object and converts any
     * constraint violations into formatted error messages that include the component name, property
     * path, and validation message.
     *
     ...
     */
    public <T> List<String> formatValidationErrors(
            T configObject, String componentName, Validator validator) {
     ...
     *
     * <p>The error message format is: {@code [ComponentName] propertyPath: validationMessage}
     *
     ...
     */
    private String formatSingleError(
            String componentName, String propertyPath, String validationMessage) {
        return "[%s] %s: %s".formatted(componentName, propertyPath, validationMessage);
    }

    /**
     * Validates multiple configuration objects and aggregates all validation errors.
     *
     * <p>This is a convenience method for validating multiple configuration objects in a single
     * call and collecting all validation errors into a single list.
     *
     * @param validator the Jakarta Bean Validation validator instance
     * @param validationTargets variable number of validation target objects containing the config
     *     object and component name
     * @return aggregated list of all validation errors from all objects; empty list if all
     *     validations pass
     * @throws IllegalArgumentException if validator is null or any validation target is invalid
     */
    public List<String> validateMultipleObjects(
            Validator validator, ValidationTarget<?>... validationTargets) {
        if (validator == null) {
            throw new IllegalArgumentException("Validator cannot be null");
        }
        ...
        for (ValidationTarget<?> target : validationTargets) {
            ...
            allErrors.addAll(
                    formatValidationErrors(
                            target.configObject(), target.componentName(), validator));
        }
     ...
     *
     * <p>This record is used with {@link #validateMultipleObjects(Validator, ValidationTarget[])}
